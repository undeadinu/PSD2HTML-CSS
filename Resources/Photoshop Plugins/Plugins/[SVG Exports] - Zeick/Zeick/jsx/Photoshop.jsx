...
svg.createSVGDesc = function ()
{
    ...
    var user = readUser();
    if (user == null || user == "") {
        //svgResult += "<text x=\"" + (docWidth >= 30 ? (docWidth / 2) : 0) + "\" y=\""  + (docHeight >= 10 ? (docHeight - 10) : 15) + 
        //    "\" fill=\"black\" font-size=\"20px\" text-anchor=\"middle\">zeick.com</text>\n";
        var w = docWidth > 0 ? docWidth : 100;
        var h = docHeight > 0 ? docHeight: 100;
        svgResult += "<path stroke='rgb( 213, 96, 77 )' stroke-width='0.62px' stroke-linecap='butt' stroke-linejoin='miter' fill='rgb( 127, 127, 127 )' " +
            "d='M0,0 C0,0 " + w + "," + h + " " + w + "," + h + "M" +w + ",0 C" + w + ",0 0," + h + " 0," + h + "  '/>";
    }
...
//        throw "'" + vd + "' doesn't match to vector format";
    ...
    for (var a in orig) {
        var cur = orig[a];
        while (cur.length != 0 && (cur[0] < '0' || cur[0] > '9')) {
            cur = cur.substr(1);
        }
        if (cur.length == 0) {
            continue;
...
generate = function() {
    try {
        var dir = Folder.selectDialog ("Directory for result");
        if (dir) {
            ...
            for(var a in selected) {
                ...
                var name = dir.fsName + "//" + cleanedName + ".svg";
                saveFile(name, data);
            }
        
            var data = "";

            params.figure_min_x.push(1000 * 1000 * 1000);
            params.figure_min_y.push(1000 * 1000 * 1000);
            params.figure_max_x.push(-1 * 1000 * 1000 * 1000);
            params.figure_max_y.push(-1 * 1000 * 1000 * 1000);
            ...
            for(var a in selected) {
                params.layerID = fixId(selected[a]);
                data = svg.createSVGDesc();
                params.needReset = false;
            }
            var name = dir.fsName + "//" + app.activeDocument.name + " (all selected layers).svg";
            if (data.length > 0)
                saveFile(name, data);
        }
